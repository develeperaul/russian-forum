<template>
  <main class="main tw-p-4 tw-pb-20">
    <div class="tw-flex tw-flex-wrap -tw-m-4">
      <div
        class="tw-w-1/6 lg:tw-w-1/6 md:tw-w-1/4 tw-p-4 tw-text-center tw-text-primary"
        v-for="icon in icons"
      >
        <base-icon class="tw-w-10 tw-h-10 tw-mx-auto" :name="icon" />
...
onMounted(() => {
  ...
  icons.value = Array.from(sprite.children).map((child) =>
    child.id.replace('icon-', '')
  )
})
